...
<html>
    ...
    <body class="desktop-detected pace-done smart-style-3 fixed-navigation fixed-header fixed-ribbon">
                                    ...
                                    <div class="widget-body" style="padding-bottom: 0px">
                                        ...
                                        <div class="tab-pane active" id="hb000">
                                            ...
                                            <div class="table-responsive">
                                                <form id="form" method="POST" >
                                                    <table class="table table-bordered table-striped" style="background-color: #f9f9f9; margin-bottom: 1px;">
                                                        <tbody>                                                            
                                                            <tr>
                                                                <td class="bg-color-blue txt-color-white text-align-right">新增廠商</td>
                                                                <td class="input-padding">
                                                                    <input class="form-control input-xs" placeholder="廠商名稱" type="text" id="vendorName" name="vendorName" required="required">
                                                                </td>
                                            ...
                                            <div id="infoBar" class="alert alert-info no-margin fade in">
                                                ...
                                                管理廠商 <code>(<span id="rowcount"></span>)</code>
                                            </div>
        ...
        <script>
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            result = result + "<tr>\n";
                            result = result + "<td><div class='col-md-12 text-center'><a class='btn btn-primary btn-sm' onClick='changeVendor(" + i + ")' id='changeBtn" + i + "'>修改</a></div></td>\n";
                            result = result + "<td id='vendor_id" + i + "'>" + data[i]['vendor_id'] + "</td>\n";
                            result = result + "<td><input class='form-control input-xs' type='text' id='changeVendor" + i + "' name='changeVendor" + i + "' value='" + data[i]['vendor_name'] + "'></td>\n";   
            ...
            {
                if ($.trim($("#changeVendor" + index).val()) == "") {
                    alert("必須填入廠商名稱");
                    return;
                }
                ...
                $('#infoBar').append("<h1 id='loadingloading' class='ajax-loading-animation'><i class='fa fa-cog fa-spin'></i> Loading...</h1></div>");
                $.ajax({
                    url: "ChangeVendor.php",
                    data: {
                        vendorId: $("#vendor_id" + index).html(),
                        changeVendor: $("#changeVendor" + index).val(),
